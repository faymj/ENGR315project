...
The prior is primarily for considerations on solar power over time, and the latter is for comparisons on temperature and solar power output. 

To adjust files being read, see lines 9 and 10 for variables ``file_path``, ``file_path2`` respectively.
...
* You are on earth (not impactful, but you never know)

No current stances on permissions of use, unless you are JMU faculty or support for them, contact us about it.